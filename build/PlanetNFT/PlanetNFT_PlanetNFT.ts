...
export function loadPlanetNFT$Data(slice: Slice) {
    const sc_0 = slice;
    return { $$type: 'PlanetNFT$Data' as const };
}
...
async function PlanetNFT_init() {
    ...
    initPlanetNFT_init_args({ $$type: 'PlanetNFT_init_args' })(builder);
...
const PlanetNFT_types: ABIType[] = [
    ...
    {"name":"PlanetNFT$Data","header":null,"fields":[]},
]
...
export class PlanetNFT implements Contract {
    ...
    static async init() {
        return await PlanetNFT_init();
    }
    
    static async fromInit() {
        const __gen_init = await PlanetNFT_init();
